...
def config():
    ...
    parser.add_argument(
        "--option", default="four", type=str, choices=["two", "four", "six"]
    )
    parser.add_argument(
        "--data-dir",
...
def main(args):
    ...
    if args.dataset == "VSR":
        ...
        result_records = dataset.evaluate_scores(
            args.model_name, scores, labels, args.output_dir, args.dataset
        )
        ...
        scores, correct_id = model.get_out_scores_wh_batched(
            args.dataset, joint_loader, args.method, args.weight, args.option
        )
        dataset.save_scores(
            scores,
            correct_id,
